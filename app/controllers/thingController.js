...
exports.findOne = (req, res) => {
    Thing.findOne({
        ...
        if (err) res.status(400).json({
            'message': 'Thing with the given ID not found.',
            'error': err
        });
        res.json(thing);
    })
...
exports.create = (req, res) => {
    Thing.create({
        ...
        if (err) res.status(400).json({
            'message': 'Something went wrong during the thing creation.',
            'error': err
        });
        res.json({
            'thing': thing,
            'message': 'Thing created with success.'
        });
    });
}
...
exports.update = (req, res, next) => {
    Thing.findOneAndUpdate({
        ...
        res.json({
            "message": "Thing saved with success.",
            'thing': thing
        });
    });
}
...
exports.delete = (req, res) => {
    Thing.findByIdAndRemove(req.body.id, (err, thing) => {
        if (err) res.status(400).json({
            'message': 'Something went wrong during the thing deletion.',
            'error': err
        });
        res.json({
